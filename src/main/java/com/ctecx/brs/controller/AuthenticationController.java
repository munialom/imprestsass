...
import com.ctecx.brs.constant.UserStatus;
import com.ctecx.brs.dto.AuthResponse;
import com.ctecx.brs.dto.UserLoginDTO;
...
public class AuthenticationController implements Serializable {
    ...
    private MasterTenant validateAndGetTenant(Integer tenantId) {
        Optional<MasterTenant> masterTenant = masterTenantService.findByClientId(tenantId);

        // Check if tenant is present and active
        MasterTenant tenant = masterTenant.orElseThrow(() ->
                new RuntimeException("Invalid tenant. Please contact service provider."));

        if (tenant.getStatus() == null ||
                tenant.getStatus().toUpperCase().equals(UserStatus.INACTIVE.name())) {
            throw new RuntimeException("Inactive tenant. Please contact service provider.");
        }
