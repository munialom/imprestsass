...
public class StockReportFormatter {

    private final ObjectMapper objectMapper;

    /**
     * Parses the raw data list from the GetStockReport stored procedure.
     * It iterates through each product record and parses the 'transactions'
     * JSON string into a nested Map object. The summary row, where 'transactions'
     * is NULL, is skipped automatically.
     *
     * @param rawDataList The raw list of results from the database.
     * @return A List of Maps representing the fully parsed stock report.
     */
    public List<Map<String, Object>> format(List<Map<String, Object>> rawDataList) {
        ...
        // Iterate over each product record in the list
        for (Map<String, Object> row : rawDataList) {
            // Parse the 'transactions' JSON field for the current row
            parseJsonField(row, "transactions", new TypeReference<Map<String, Object>>() {});
        }
    ...
    private <T> void parseJsonField(Map<String, Object> dataMap, String key, TypeReference<T> typeRef) {
        ...
        // This check correctly handles NULL values (like in the summary row) and empty strings.
        if (value instanceof String jsonString && !jsonString.trim().isEmpty()) {
            try {
                ...
                // If parsing fails, log the error and set an empty map as a safe default.
